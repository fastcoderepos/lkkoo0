	...
	protected BooleanBuilder searchKeyValuePair(QRentalEntity rental, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			...
			if(details.getKey().replace("%20","").trim().equals("returnDate")) {
				if(details.getValue().getOperator().equals("equals") && SearchUtils.stringToLocalDate(details.getValue().getSearchValue()) !=null)
					builder.and(rental.returnDate.eq(SearchUtils.stringToLocalDate(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("notEqual") && SearchUtils.stringToLocalDate(details.getValue().getSearchValue()) !=null)
					builder.and(rental.returnDate.ne(SearchUtils.stringToLocalDate(details.getValue().getSearchValue())));
				else if(details.getValue().getOperator().equals("range"))
				{
				   LocalDate startLocalDate= SearchUtils.stringToLocalDate(details.getValue().getStartingValue());
				   LocalDate endLocalDate= SearchUtils.stringToLocalDate(details.getValue().getEndingValue());
				   if(startLocalDate!=null && endLocalDate!=null)	 
					   builder.and(rental.returnDate.between(startLocalDate,endLocalDate));
				   else if(endLocalDate!=null)
					   builder.and(rental.returnDate.loe(endLocalDate));
                   else if(startLocalDate!=null)
                	   builder.and(rental.returnDate.goe(startLocalDate));  
                 }
